...
pub fn total_cost(item_quantity: &str) -> Result<i32, ParseIntError> {
    ...
    let cost_per_item = 5;
    // let qty = item_quantity.parse::<i32>();//the result is a Result<T, E>,Ok(i32)或Err(ParseIntError)
    let qty = item_quantity.parse::<i32>()?; //使用？，如果解析成功，返回解析的值，如果失败，返回Err(ParseIntError)
